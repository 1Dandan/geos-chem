...
while [ "${valid_met}" -eq 0 ]; do
	...
	fp_msg="WARNING: The convection scheme used to generate archived GEOS-FP meteorology \nfiles changed from RAS to Grell-Freitas starting June 1 2020 with impact on \nvertical transport. Discussion and analysis of the impact is available at \ngithub.com/geoschem/geos-chem/issues/1409. See also NASA GMAO documentation \non the switch at: \nhttps://gmao.gsfc.nasa.gov/researchbriefs/new_atmos_phys_GEOS-FP/new_atmos_phys_GEOS-FP.pdf.\n\nIn addition, there is a bug in convective precipitation flux following the \nswitch where all values are zero in the input files. This bug is addressed \nby computing fluxes online for runs starting on or after June 1 2020. The \nfix does not extend to the case of running across the time boundary.\n\nDue to these issues we recommend splitting up GEOS-FP runs in time such that\na single simulation does not span the switch. Configure one run to end on \nJune 1 2020 and then use its output restart to start another run on June 1. \nAlternatively consider using MERRA2. If you wish to use a GEOS-FP meteorology \nyear different from your simulation year please create a GEOS-Chem GitHub \nissue for assistance to avoid accidentally using zero convective precipitation flux.\n"
	printf "\n${fp_msg}\n"
